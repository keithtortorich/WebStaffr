...
- **Open, flagged conflict:** DECISIONS.md records an active decision that the first system slice (internal workflow execution) explicitly excludes "any stack/hosting/database choice." The founder's 2026-07-04 autonomy directive describes Phase 1 as including core infrastructure (auth, DB, API, CI/CD, secrets, testing, monitoring, deployment). These two are in tension and have not yet been reconciled — see the founder-decision request raised in-session. No infrastructure or stack work proceeds until this is resolved.
