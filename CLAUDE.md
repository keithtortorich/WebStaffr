...
- SQLite persistence added for WorkflowDefinitions and ExecutionRecords (`webstaffr/db.py`, `webstaffr/repository.py`, `webstaffr/migrations/`), stdlib-only, tenant-scoped, no new dependency. 21/21 tests passing; health check HEALTHY. See DECISIONS.md, "Add SQLite... as the persistence mechanism." Still no auth, API, CI/CD, hosting, or deployment decision.
