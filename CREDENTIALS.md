...
# 2. Run with real backends (uvicorn does not auto-load .env -- export
# first, or use a tool like `direnv` / `python-dotenv` if you want that)
export $(cat .env | xargs)
./.venv/bin/uvicorn webstaffr.workers.angel.router:app --reload

# Or via Docker -- docker compose auto-loads a .env file from the repo
# root by default, no extra flag needed:
...
- Run the full suite: `./.venv/bin/python -m unittest discover -s tests`
- Run the self-healing health check: `./.venv/bin/python scripts/health_check.py`
...
- Watch logs for `VoiceBackendNotConfiguredError` / `GHLNotConfiguredError`
  ...
  retrying).
- No hosting/deployment decision has been made yet for this repo -- see
  `DECISIONS.md`. `Dockerfile`/`docker-compose.yml` exist for local
  testing only and have not been build-verified (no Docker available in
  the environment they were authored in).
