...
Execution model (per ARCHITECTURE.md): sequential, single-instance,
run-to-completion. No concurrency, scheduling, or multi-instance
coordination in this slice.
...
the excluded "retry/scheduling" — that refers to re-scheduling or
re-coordinating whole workflow *instances*, which this slice still does not
do. Kept conservative and off by default (max_attempts_per_step=1) so no
hidden complexity is introduced beyond what was asked for.
"""
