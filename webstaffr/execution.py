...
reconstruct what happened in any given run. `to_dict`/`from_dict` here are
the single source of truth for that round trip -- the repository layer
just moves the dict shape to and from SQLite (as JSON) rather than knowing
the record's internal structure itself.
"""
