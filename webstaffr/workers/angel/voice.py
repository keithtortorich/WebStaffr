## OWNED_BY: GROK (extended from Claude base; see DECISIONS.md for the
## integration review/fixes applied when merging Grok's implementation)
...
class GrokVoiceBackend:
    ...
    Implemented by Grok (the founder's second AI assistant, coordinating
    on this repo per the founder-mediated protocol recorded in
    DECISIONS.md) as an explicitly-invited extension of this
    otherwise-Claude-owned file. A synchronous request/response chat
    completion is enough to satisfy this class's
    `respond(message, context) -> str` contract; live realtime audio
    streaming remains separate future work if/when Angel needs actual
    voice rather than text chat.
